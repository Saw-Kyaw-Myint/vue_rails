<template>
  ...
  <div id="password-change" class="" v-if="changePassword">
    <p class="x" @click="changePassword = false">
      <i class="fa-solid fa-x"></i>
    </p>
    ...
    <form @submit.prevent="passwordUpdate">
      ...
      <div class="cps-input-group">
        ...
        />
        <span class="error-message">{{
          passwordErrors?.new_password ? passwordErrors.new_password[0] : " "
        }}</span>
      </div>
      <div class="cps-input-group">
...
watchEffect(async () => {
  await axios
    ...
    .then((response) => {
      ...
      posts.value = response.data.user.posts;
    });
...
const postDelete = async (id) => {
  await axios
    .delete(`${import.meta.env.VITE_PUBLIC_API_URL}/posts/${id}`)
    .then((response) => {
